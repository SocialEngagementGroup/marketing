    ...
    <section ref={sectionRef} className="bg-white py-20">
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
        <div className="text-center mb-12">
          ...
          >
            See How We <br className="md:hidden" /> <span className="text-[#0A5D46]">Transform</span> Businesses
          </motion.h2>
        </div>
        ...
        >
          <iframe 
            ref={iframeRef}
            src="https://www.youtube.com/embed/ujt54JDgbYo?rel=0&modestbranding=1&enablejsapi=1&controls=0&iv_load_policy=3&loop=1&playlist=ujt54JDgbYo" 
          ...
          <div className={`absolute inset-0 z-20 flex items-center justify-center transition-all duration-500 ${isPlaying ? 'bg-transparent lg:group-hover:bg-black/10' : 'bg-black/10'}`}>
            <div className={`w-20 h-20 sm:w-24 sm:h-24 bg-[#064232]/90 rounded-full flex items-center justify-center text-white shadow-2xl transform transition-all duration-500 ${isPlaying ? 'opacity-0 lg:group-hover:opacity-100 scale-90' : 'opacity-100 scale-100 animate-pulse'}`}>
              {isPlaying ? (
