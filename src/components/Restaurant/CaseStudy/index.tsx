import React from 'react';
import { ArrowUpRight, TrendingUp, UtensilsCrossed, MapPin } from 'lucide-react';
import { motion } from 'framer-motion';

import CountUp from '../../Common/ui/CountUp';

const CaseStudy: React.FC = () => {
  const metrics = [
    { label: "Google Visibility", value: 70, suffix: "%", prefix: "+" },
    { label: "Reservation Requests", value: 55, suffix: "%", prefix: "+" },
    { label: "Google Maps Ranking", value: 3, prefix: "Top " }
  ];

  return (
    <section className="py-20 md:py-32 bg-[#A64942] relative overflow-hidden">
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-20 items-center">
          
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
          >
            <span className="text-white/80 font-bold tracking-widest text-sm uppercase block mb-4">Featured Success Story</span>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-10 leading-tight font-outfit">
              Flame Japanese <br /> 
              Hibachi & Grill
            </h2>
            
            <div className="space-y-8 text-white/60 font-medium mb-12 lg:mb-12">
              <div>
                <p className="text-white font-bold uppercase text-xs tracking-widest mb-3">The Challenge:</p>
                <p className="text-base text-white/80 border-l-2 border-white/10 pl-6 font-outfit">A top-tier hibachi spot with a dated website, zero review strategy, and virtually no presence on Google Maps despite great food.</p>
              </div>
              
              <div>
                <p className="text-white font-bold uppercase text-xs tracking-widest mb-3">Our Solution:</p>
                <p className="text-base text-white/80 border-l-2 border-[#B95D56]/50 pl-6 font-outfit">Mobile-first menu redesign, Google Maps optimization, automated review requests, and hyper-local Search Ads.</p>
              </div>
            </div>
          </motion.div>

          {/* Metrics Grid */}
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-6 lg:-mt-12">
            {metrics.map((metric, index) => {
              const Icon = index === 0 ? TrendingUp : index === 1 ? UtensilsCrossed : MapPin;
              return (
                <motion.div 
                  key={index}
                  initial={{ opacity: 0, x: 30 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: index * 0.1, duration: 0.6 }}
                  className="bg-white/5 backdrop-blur-sm p-8 rounded-[2.5rem] border border-white/10 flex flex-col justify-center items-start lg:items-center text-left lg:text-center group hover:bg-white/10 transition-all duration-300 relative overflow-hidden lg:min-h-[300px]"
                >
                  <div className="relative z-10 flex flex-col">
                    <h3 className="text-4xl sm:text-5xl font-bold text-white font-outfit">
                      <CountUp end={metric.value} prefix={metric.prefix} suffix={metric.suffix} />
                    </h3>
                    <p className="text-white/40 text-xs font-bold uppercase tracking-widest mt-2 font-outfit">{metric.label}</p>
                  </div>

                  {/* Background Icon Watermark */}
                  <div className="absolute -right-6 -bottom-6 opacity-10 transform rotate-[-15deg] group-hover:scale-110 transition-transform duration-500 pointer-events-none">
                    <Icon className="text-white w-[160px] h-[160px]" />
                  </div>
                </motion.div>
              );
            })}
          </div>

        </div>
      </div>
    </section>
  );
};

export default CaseStudy;
